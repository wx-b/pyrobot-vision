...
class Evaluator(object):
	def __init__(self, dataset_path, body_names, sequence_name, save_path, image_handle):
		...
		self.image_handle = self.image_handle_initializer(self.dataset_path, self.body_names[0], self.sequence_name)
		...
		self.body = Body("body", filesystem("path_placeholder"), 0.001, True, False, 0.3)
		self.model = Model("model")
		self.model.set_use_random_seed(False)
		self.regional_modality = RegionModality()
		self.regional_modality.Init("regional_modality", self.body, self.model, self.camera)
	...
	def Evaluate(self):
		...
		for i_body in range(len(self.body_names)):
			...
			for i_frame in range(self.kNFrames - 1):
				...
				result[1:4] = self.CalculatePoseResults(self.body.body2world_pose(), self.poses_first[i_frame + 1])

				self.ResetBody(i_frame+1)

			single_body_result = results[i_body]
			tracking_loss = single_body_result[:, 3].sum() / single_body_result[:, 3].size
			translation_error = single_body_result[:, 1].sum() / single_body_result[:, 1].size
			rotation_error = single_body_result[:, 2].sum() / single_body_result[:, 2].size
			complete_cycle = single_body_result[:, 4].sum() / single_body_result[:, 4].size
			calculate_before_camera_update = single_body_result[:, 5].sum() / single_body_result[:, 5].size
			calculate_correspondences = single_body_result[:, 6].sum() / single_body_result[:, 6].size
			calculate_pose_update = single_body_result[:, 7].sum() / single_body_result[:, 7].size

			single_body_path = os.path.join(self.save_path, "result_{}.txt".format(self.body_names[i_body]))
			single_body_dict = {
	...
	def CalculatePoseResults(self, pose, pose_gt):
		...
		tracking_loss = int(translation_error > self.translation_error_threshold or rotation_error > self.rotation_error_threshold)
	...
	def InitBodies(self, body_name):
		self.image_handle = self.image_handle_initializer(self.dataset_path, body_name, self.sequence_name)
		self.tracker.image_handle = self.image_handle
		self.camera.UpdateImage2(self.image_handle.get_image())
		self.body.set_name(body_name)
		self.body.set_geometry_path(filesystem(os.path.join(self.dataset_path, body_name, "{}.obj".format(body_name))))
		...
		if not self.model.LoadModel(model_directory, model_name):
			self.model.GenerateModel(self.body, self.sphere_radius, self.n_divides, self.n_points)
