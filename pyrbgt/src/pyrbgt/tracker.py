		...
		for region_modality in self.region_modalities:
			if region_modality.camera_ptr():
				if region_modality.camera_ptr().name() not in self.cameras.keys():
					self.cameras[region_modality.camera_ptr().name()] = region_modality.camera_ptr()
			if region_modality.occlusion_mask_renderer_ptr():
				if region_modality.occlusion_mask_renderer_ptr().name() not in self.occlusion_renderers.keys():
					self.occlusion_renderers[region_modality.occlusion_mask_renderer_ptr().name()] = region_modality.occlusion_mask_renderer_ptr()
