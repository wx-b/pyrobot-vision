...
class RBGTTracker(object):
	...
	def track(self, image_handle):
		...
		self.backend_tracker_thread = subThreadTracker(self.image_handle, self.configs, self.output_lst, self.threadLock, self.visualize, self.evaluate)
...
class subThreadTracker(threading.Thread):
	def __init__(self, image_handle, configs, output_lst, threadLock, visualize=True, evaluate=True):
		...
		if self.evaluate:
			...
			poses_first_file = open(os.path.join(self.configs.evaluate_dataset_path, 'poses_first.txt'), 'r')
			...
			poses_second_file = open(os.path.join(self.configs.evaluate_dataset_path, 'poses_second.txt'), 'r')
		...
		self.camera.set_intrinsics(intrinsics.fu, intrinsics.fv, intrinsics.ppu, intrinsics.ppv, intrinsics.width, intrinsics.height)
		...
		self.occlusion_mask_renderer.Init("occlusion_mask_renderer", self.renderer_geometry, self.camera.world2camera_pose(), self.camera.intrinsics(), 0.01, 5.0)

		occlusion_mask_id = 1
		for model_config in self.model_configs:
			body = Body(model_config.name, filesystem(model_config.path), 0.001, True, False, 0.3)
			...
			regional_modality.Init("regional_modality", body, model, self.camera)
			regional_modality.UseOcclusionHandling(self.occlusion_mask_renderer)

			self.tracker.AddRegionModality(regional_modality)

			if not model.LoadModel(filesystem(model_config.path), model_config.name + "_model"):
				model.GenerateModel(body, 0.8, 4, 200)
				model.SaveModel(model_config.path, model_config.name + "_model")

			self.bodies[model_config.name] = body
			original_pose = Transform3fA()
			mat2t3fAfullMat(model_config.original_pose, original_pose)
			body.set_body2world_pose(original_pose)
			...
			if self.evaluate:
				for key, body in self.bodies.items():
					if key == "squirrel_small":
						self.poses_second_error.append(self.CalculatePoseResults(body.body2world_pose(), self.poses_second[i_frame + 1]))
					else:
						self.poses_first_error.append(self.CalculatePoseResults(body.body2world_pose(), self.poses_first[i_frame + 1]))
				self.ResetBody(i_frame+1)

				if i_frame >= 999:
					result = np.mean(self.poses_first_error, axis=0)
					print({"translation_loss": result[0], "rotation_loss": result[1], "success_rate": 1-result[2]})
					break

			for key, body in self.bodies.items():
				body2world_pose = np.array(t3fA2mat(body.body2world_pose())).reshape((4,4))
				self.output_lst.append({"name": key, "translation": body2world_pose[:3, 3], "rotation": body2world_pose[:3, :3]})
	...
	def CalculatePoseResults(self, pose, pose_gt):
		...
		rotation_error = math.acos(np.clip((np.matmul(rotation.T, rotation_gt).trace() - 1) / 2, -1, 1))
		tracking_loss = int(translation_error > 0.05 or rotation_error > 5 * math.pi / 180)
